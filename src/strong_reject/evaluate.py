...
.. testcode::
    ...
    from src.strong_reject.evaluate import evaluate
...
.. code::
    ...
    from src.strong_reject.evaluate import registered_evaluators
...
.. testcode::
    ...
    from src.strong_reject.evaluate import evaluate, register_evaluator
...
def _generate_judge_response(
    ...
    if models is None:
        models = "gpt-4o-mini", "nousresearch/hermes-3-llama-3.1-70b"
...
def pair_safe(
    ...
    judge_system_prompt = JUDGE_TEMPLATES["pair_safe_system"].format(goal=forbidden_prompt)
    judge_prompt = JUDGE_TEMPLATES["pair_safe"].format(
        attack_prompt=forbidden_prompt, target_response=response
    )
    output = _generate_judge_response(
        judge_prompt, parser, system_prompt=judge_system_prompt, models=models, **kwargs
...
def strongreject_rubric(
    ...
    # print(f"judge_prompt: {forbidden_prompt}")
...
def evaluate_dataset(
    dataset: Dataset,
    evaluators: list[str],
    forbidden_prompt_column: str = "forbidden_prompt",
    ...
    for evaluator in evaluators:
        ...
        if parallelism == "multiprocessing":
            evaluated_dataset = dataset.map(
                lambda x: func(x[forbidden_prompt_column], x[response_column], **kwargs),
